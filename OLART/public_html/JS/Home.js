...
function gerarCardDinamico() {
    ...
    for (let i = 0; i < artes.length; i++) {
        ...
        count += '<img class="img-cadastro" src="../imagens/artes/' + '2.png' + ' > ';
        count += '</div>';
        count += '<div class="infos" id="id_infos">';
        count += '<h2 class="titulo-arte">' + 'artes' + '</h2>';
        count += '<hr class="separador-card">';
        count += '<h2 class="descricao-titulo">Sobre</h2>';
        count += '<div class="box-descricao">';
        count += '<p class="descricao-card">A arte é uma representação</p>';
        count += '</div>';
        count += '<h2 class="preco">R$ 1.000</h2>';
        count += '</div>';
        count += '</div>';
    ...
    document.getElementById("gerar").innerHTML = count;
}